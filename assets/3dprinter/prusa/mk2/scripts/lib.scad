...
module prusa_frame(size, frame_type) {
    if (frame_type == "ANET_A8") { anet_a8_frame(size, extrusion_by_name(profile_type)); }
    else if (frame_type == "P35X70") { p35x70_frame(size); }
}
...
module prusa_main() {
    ...
    else if (part == "frame") { prusa_frame(size, frame); }
}
